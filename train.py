...
def compute_metrics(pred, target):
    ...
    if wandb.config.loss_type == "regression":
        pred_classes = classify_movement(
            pred,
            wandb.config.classify_threshold_up,
            wandb.config.classify_threshold_down,
        )
    ...
    target_classes = classify_movement(
        target, wandb.config.classify_threshold_up, wandb.config.classify_threshold_down
    )
...
def main():
    ...
    val_loader = DataLoader(
        val_ds, batch_size=config.batch_size, collate_fn=MovementDataset.coll_samples
    )
    test_ds = MovementDataset(X_test, config.transformer_model)
    test_loader = DataLoader(
        test_ds, batch_size=config.batch_size, collate_fn=MovementDataset.coll_samples
    )
    ...
    for epoch in tqdm(range(config.epochs)):
        ...
        with torch.no_grad():
            val_metrics = evaluate(
                model, val_loader, criterion, device, metric_prefix="val"
            )
            test_metrics = evaluate(
                model, test_loader, criterion, device, metric_prefix="test"
            )
